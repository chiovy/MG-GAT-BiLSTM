...
# -*- coding: utf-8 -*-
"""
MG-GAT-BiLSTM Model (Ablation: No MIC Graph)

This module implements the MG-GAT-BiLSTM model without MIC graph.
The model uses two graph branches (dCor, Spearman) and Global Attention + BiLSTM for temporal modeling.
"""
...
class MG_GAT_BiLSTM_NoMIC(nn.Module):
    """
    Multi-Graph GAT-BiLSTM Model (Ablation: No MIC Graph)
    
    This model uses two graph branches (dCor, Spearman) for multi-graph fusion,
    ...
    def __init__(self, num_nodes: int, node_in_dim: int = 1, node_emb_dim: int = 32,
        ...
        self.branch_sp = GATBranch(node_emb_dim, node_emb_dim, gat_heads, gat_dropout)
    ...
    def forward(self, x, g_dcor, g_sp):
        """
        Forward pass.
        ...
        Args:
            ...
            g_sp: Tuple of (edge_index, edge_weight) for Spearman graph
        ...
        h1 = self.branch_dcor(x, *g_dcor)
        h2 = self.branch_sp(x, *g_sp)

        w = torch.softmax(self.gate, dim=0)
        h = w[0] * h1 + w[1] * h2
        h = self.gatt(h)

        seq = h.reshape(h.size(0), h.size(1), -1)
        out, _ = self.rnn(seq)
