...
# -*- coding: utf-8 -*-
"""
MG-GAT-BiLSTM Model (Ablation: No Global Attention)

This module implements the MG-GAT-BiLSTM model without Global Attention mechanism.
The model uses three graph branches (MIC, dCor, Spearman) and BiLSTM for temporal modeling.
"""
...
class MG_GAT_BiLSTM_NoAttention(nn.Module):
    """
    Multi-Graph GAT-BiLSTM Model (Ablation: No Global Attention)
    
    This model uses three graph branches (MIC, dCor, Spearman) for multi-graph fusion,
    but removes the Global Attention mechanism. The fused features are directly fed
    into BiLSTM for temporal modeling.
    ...
    def __init__(self, num_nodes: int, node_in_dim: int = 1, node_emb_dim: int = 32, 
        ...
        # Three GAT branches (MIC, dCor, and Spearman)
        ...
        self.branch_spearman = GATBranch(node_emb_dim, node_emb_dim, gat_heads, gat_dropout)
        
        # Learnable fusion gate (3 branches)
        self.gate = nn.Parameter(torch.zeros(3))
        
        # No Global Attention (removed)
    ...
    def forward(self, x, g_mic, g_dcor, g_spearman):
        """
        Forward pass.
        ...
        Args:
            ...
            g_spearman: Tuple of (edge_index, edge_weight) for Spearman graph
        ...
        h_spearman = self.branch_spearman(x, *g_spearman)
        
        # Learnable fusion
        w = torch.softmax(self.gate, dim=0)
        h = w[0]*h_mic + w[1]*h_dcor + w[2]*h_spearman  # [B,T,N,D]
        
        # No Global Attention (directly to BiLSTM)
